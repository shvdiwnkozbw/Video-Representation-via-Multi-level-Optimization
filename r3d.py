...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.melow = nn.Sequential(
            MEBlock(channel=128,reduction=16),
            nn.AdaptiveAvgPool3d((8, 1, 1))
        )
        self.memid = nn.Sequential(
            MEBlock(channel=256,reduction=16),
            nn.AdaptiveAvgPool3d((4, 1, 1))
        )

        self.avgpool = nn.AdaptiveAvgPool3d((2, 1, 1))
    ...
    def temporal_shuffle(self, x):
        n, c, t, h, w = x.shape
        x1 = torch.flip(x, dims=(2,))
        return torch.cat([x, x1], dim=0)
    ...
    def forward(self, x):
        ...
        feat = self.avgpool(high).flatten(2)
        low = self.temporal_shuffle(low)
        mid = self.temporal_shuffle(mid)
        melow = self.melow(low).flatten(2)
        memid = self.memid(mid).flatten(2)

        return feat, melow, memid
